...
public:
	...
	// Insert x into the tree; duplicates are igored
	void insert(const Comparable & x) { insert(x, root); }

	void insert(Comparable && x) { insert(std::move(x), root); }

	// Remove x from the tree. Nothing is done if x is not found.
	void remvoe(const Comparable & x) { remove(x, root); }
...
private:
	...
	/*
	 * Internal method to find the largest item is a subtree t.
	...
	*/
	 ...
	 * For AVL trees, this a single rotation for case 1.
	 ...
	 * Update heights, the set new root.
	 */
	 ...
	 * For AVL trees, this a double rotation for case 2.
	 * Update heights, then set new root.
	 */
